...
void loop( void ){
    ...
    //  myTimer01.start(mySerialRead, 0, 200);  // replace this timer
    ...
    // myTimer02.start(mySomethingWrong, 5000);  // test if answer in time
    if ( s1done && (millis()-s1) > 5000) {
       s1 = millis();
       s1done = false;
       mySomethingWrong();
       Serial.println("-5- SECONDS ONE SHOT ONLY");
       delay(30);
    }
