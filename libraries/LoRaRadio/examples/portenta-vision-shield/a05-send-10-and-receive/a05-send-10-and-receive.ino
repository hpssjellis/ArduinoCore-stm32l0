...
void setup( void )
{
   ...
   // timerOn.start(callbackOn,  3000);
    ...
    //LoRaRadio.receive(0);    // if 1000 receinve for only 1 second. if 0 receive continuously

    myTimer02.start(mySend, 0, 7000);
    
 // LoRaRadio.setIQInverted(true);   //note false is standard
...
static void myReceive(void){
  ...
  while (LoRaRadio.available() ) {
    ...
    if (myI >= myCharMax) {break;}
     ...
     myTimer01.start(myPrint, 300);   // only calls once after 5 ms,  if  0, 1000 would continuously call every 1 second.
}
